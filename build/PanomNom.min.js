...
a=1);this.zoom=a;this.canvas.width=this.widths[a];this.canvas.height=this.heights[a];var b=this.levelsW[a],f=this.levelsH[a];this.service.getPanoramaById(c,function(a,c){null===a?this.error("Can't load panorama information"):(this.metadata=a,this.dispatchEvent({type:"data",message:a}))}.bind(this));for(var e=0;e<f;e++)for(var g=0;g<b;g++)this.stitcher.addTileTask({url:"https://geo0.ggpht.com/cbk?cb_client=maps_sv.tactile&authuser=0&hl=en&panoid="+c+"&output=tile&x="+g+"&y="+e+"&zoom="+a+"&nbt&fover=2",
x:512*g,y:512*e});this.stitcher.processQueue()};a.GoogleStreetViewLoader.prototype.loadFromLocation=function(a,d){this.getIdByLocation(a,function(a){this.load(a,d)}.bind(this))};a.GoogleStreetViewLoader.prototype.loadFromURL=function(a,d){if(-1!=a.indexOf("panoid")){var b;a:{b=a.split("&");for(var f=0;f<b.length;f++){var e=b[f].split("=");if("panoid"==decodeURIComponent(e[0])){b=decodeURIComponent(e[1]);break a}}b=void 0}this.load(b,2)}else-1!=a.indexOf("!1s")?(b=a.indexOf("!1s")+3,f=a.substr(b).indexOf("!"),
b=a.substr(b,f),this.load(b,d)):this.dispatchEvent({type:"error",message:"can't find panorama id in specified URL"})};a.GoogleStreetViewLoader.prototype.getIdByLocation=function(a,d){var b="https://cbks0.google.com/cbk?cb_client=maps_sv.tactile&authuser=0&hl=en&output=polygon&it=1%3A1&rank=closest&ll="+a.lat()+","+a.lng()+"&radius=50",f=new XMLHttpRequest;f.open("GET",b,!0);f.onreadystatechange=function(){if(4==f.readyState&&200==f.status){var a=JSON.parse(f.responseText);a&&a.result&&0!==a.result.length?
d(a.result[0].id):this.error("No panoramas around location")}}.bind(this);f.send(null)};a.GooglePhotoSphereLoader=function(){a.Loader.call(this);this.service=a.Utils.getGoogleStreetViewService()};a.GooglePhotoSphereLoader.prototype=Object.create(a.Loader.prototype);a.GooglePhotoSphereLoader.prototype.loadFromURL=function(a,d){if(-1!=a.indexOf("!1s")){var b=a.indexOf("!1s")+3,f=a.substr(b).indexOf("!"),b=a.substr(b,f);this.load(b,d)}};a.GooglePhotoSphereLoader.prototype.load=function(a,d){this.zoom=
d;this.service.getPanoramaById(a,function(b,f){if(null===b)this.error("Can't load panorama information");else{var e=Math.floor(Math.log(b.tiles.worldSize.width/b.tiles.tileSize.width)/Math.log(2));this.canvas.width=b.tiles.worldSize.width*Math.pow(2,d-1)/Math.pow(2,e);this.canvas.height=b.tiles.worldSize.height*Math.pow(2,d-1)/Math.pow(2,e);for(var e=this.canvas.width/b.tiles.tileSize.width,g=this.canvas.height/b.tiles.tileSize.height,h=0;h<g;h++)for(var k=0;k<e;k++)this.stitcher.addTileTask({url:"https://geo1.ggpht.com/cbk?cb_client=maps_sv.tactile&authuser=0&hl=en&panoid="+
a+"&output=tile&x="+k+"&y="+h+"&zoom="+d+"&nbt&fover=2",x:k*b.tiles.tileSize.width,y:h*b.tiles.tileSize.height});this.stitcher.processQueue()}}.bind(this))};window.PANOMNOM=a})();
